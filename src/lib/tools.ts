...
export const tools: Tool[] = [
  ...
  },
  {
    ...
    description: "Merge, split, reorder PDFs, render PDF pages to images, convert Markdown to PDF, and combine images into a PDF from one local browser workspace.",
    category: "file-media",
    tier: 1,
    seoTitle: "PDF Toolkit | Merge Split Reorder PDF, PDF to Image, Markdown to PDF",
    seoDescription: "A browser-local PDF toolkit for merging and splitting PDFs, reordering page ranges, converting PDF pages to PNG/JPG, creating PDFs from images, and converting Markdown to PDF.",
    inputs: [
      { label: "PDF, image, or Markdown", description: "Upload PDFs for merge/split or page rendering, PNG/JPG images for image-to-PDF, or paste Markdown for Markdown-to-PDF." },
    ...
    examples: [
      ...
      { label: "Markdown brief", input: "Headings, lists, code sample", output: "Printable PDF" },
    ],
    explanation: [
      "The PDF Toolkit groups the PDF workflows that users usually need together: combine documents, pick page ranges, render pages as images, turn image files into a PDF, and convert Markdown notes into a printable PDF.",
    ...
    relatedToolSlugs: ["markdown-to-pdf", "image-format-converter", "og-image-generator", "qr-barcode-generator"],
    ko: {
      title: "PDF 도구 모음",
      shortTitle: "PDF 도구",
      description: "PDF 병합·분할·재정렬, PDF 페이지 이미지 변환, 이미지 여러 장 PDF 생성, Markdown → PDF, 텍스트 추출, 페이지 삭제, 2-up 병합, 여백 자르기를 한 메뉴에서 처리합니다.",
      seoTitle: "PDF 도구 모음 | PDF 병합 분할 재정렬, Markdown PDF, 텍스트 추출",
      seoDescription: "PDF 병합/분할/재정렬, PDF 페이지 PNG/JPG 변환, 이미지 PDF 생성, Markdown → PDF, 텍스트 추출, 페이지 삭제, 2-up 병합, 여백 자르기를 브라우저 로컬 처리로 제공합니다.",
      inputs: [
        { label: "PDF, 이미지 또는 Markdown", description: "병합/분할/페이지 이미지 변환에는 PDF를, 이미지 PDF 생성에는 PNG/JPG를, Markdown → PDF에는 텍스트 또는 .md 파일을 사용합니다." },
      ...
      examples: [
        ...
        { label: "Markdown 브리프", input: "제목, 목록, 코드 샘플", output: "인쇄용 PDF" },
      ],
      explanation: [
        "PDF 도구 모음은 실제 사용자가 한 세션에서 자주 이어서 쓰는 작업을 한 페이지에 묶었습니다. 문서 합치기, 페이지 범위 선택, 페이지 이미지 변환, 이미지 파일 PDF 묶기, Markdown → PDF를 탭으로 전환합니다.",
      ...
      faqs: [
        { question: "파일이 서버로 업로드되나요?", answer: "아닙니다. PDF/이미지/Markdown 처리 탭은 브라우저에서 실행됩니다. 다른 네트워크 진단 도구와 달리 이 PDF 도구 모음은 파일을 업로드하지 않습니다." },
        { question: "PDF 도구를 왜 한 메뉴로 합쳤나요?", answer: "PDF 작업은 병합, 분할, 이미지 변환, 이미지 PDF 생성, Markdown 변환이 한 흐름에서 자주 이어집니다. 탭 구조가 메뉴를 늘리지 않으면서 기능을 찾기 쉽게 만듭니다." },
  ...
  },
  {
    ...
    relatedToolSlugs: ["markdown-to-pdf", "text-case-converter"],
    ko: {
      title: "마크다운 표 생성기",
      shortTitle: "마크다운 표",
  ...
  },
  {
    ...
    shortTitle: "MD → PDF",
    description: "Convert Markdown notes, README drafts, and simple docs into a printable PDF entirely in the browser.",
    category: "file-media",
    tier: 1,
    seoTitle: "Markdown to PDF | Browser-Local MD Converter",
    seoDescription: "Convert Markdown to PDF locally in your browser. Supports headings, lists, quotes, code blocks, simple tables, Korean text, A4/Letter pages, and no server upload.",
    inputs: [
      { label: "Markdown text or .md file", description: "Paste Markdown or load a .md/.txt file. Headings, lists, quotes, fenced code, and simple tables are supported." },
      { label: "Page options", description: "Choose A4 or Letter, margin width, optional cover title, and output file name." },
    ],
    outputs: [
      { label: "PDF download", description: "A multi-page PDF rendered with the device system font, ready to save or print." },
      { label: "Build summary", description: "Page count and file size after conversion." },
    ],
    examples: [
      { label: "Meeting notes", input: "H1 title, bullets, action list", output: "2–3 page A4 PDF" },
      { label: "README draft", input: "Headings, code fence, table", output: "Printable PDF without upload" },
      { label: "Korean brief", input: "한글 제목과 목록", output: "System-font PDF with CJK glyphs" },
    ],
    explanation: [
    ...
    faqs: [
      { question: "Is Markdown uploaded to a server?", answer: "No. Parsing and PDF creation run in your browser. Nothing is sent for the conversion itself." },
      { question: "Are images embedded?", answer: "No. Image Markdown becomes link/alt text only. For image-heavy PDFs, use the Images to PDF tab in the PDF Toolkit." },
      { question: "Why does Korean look different on some devices?", answer: "Text is drawn with the device system font stack. Available fonts vary by OS and browser, so spacing and glyph style can differ slightly." },
      { question: "Is this a full Markdown print engine?", answer: "No. It covers everyday notes: headings, paragraphs, lists, quotes, fenced code, horizontal rules, and simple tables." },
    ],
    relatedToolSlugs: ["pdf-toolkit", "markdown-table-generator", "developer-text-toolkit", "og-image-generator"],
    ko: {
      title: "Markdown → PDF",
      shortTitle: "MD → PDF",
      description: "Markdown 메모, README 초안, 짧은 문서를 브라우저에서 PDF로 변환합니다. 서버 업로드 없이 로컬 처리합니다.",
      seoTitle: "Markdown to PDF | 마크다운 PDF 변환기",
      seoDescription: "Markdown을 브라우저 로컬에서 PDF로 변환합니다. 제목·목록·인용·코드·간단한 표, 한글, A4/Letter, 업로드 없음.",
      inputs: [
        { label: "Markdown 텍스트 또는 .md 파일", description: "Markdown을 붙여넣거나 .md/.txt 파일을 불러옵니다. 제목, 목록, 인용, 코드 블록, 간단한 표를 지원합니다." },
        { label: "페이지 옵션", description: "A4/Letter, 여백, 선택 표지 제목, 출력 파일명을 지정합니다." },
      ],
      outputs: [
        { label: "PDF 다운로드", description: "기기 시스템 폰트로 렌더링된 여러 페이지 PDF를 저장합니다." },
        { label: "생성 요약", description: "변환 후 페이지 수와 파일 크기를 표시합니다." },
      ],
      examples: [
        { label: "회의록", input: "H1 제목, 불릿, 액션 리스트", output: "A4 2–3페이지 PDF" },
        { label: "README 초안", input: "제목, 코드 블록, 표", output: "업로드 없는 인쇄용 PDF" },
        { label: "한글 브리프", input: "한글 제목과 목록", output: "CJK 글리프가 포함된 PDF" },
      ],
      explanation: [
        "Markdown은 초안에 편하지만 공유·인쇄에는 고정 레이아웃 PDF가 필요할 때가 많습니다. 이 도구는 변환을 브라우저 로컬로 유지해 서버 왕복 없이 메모를 PDF로 만듭니다.",
        "렌더링은 Canvas 텍스트 레이아웃과 pdf-lib 패키징을 사용합니다. 라틴 전용 표준 PDF 폰트만 쓰는 방식보다 한글 등 시스템 폰트 글리프를 다루기 쉽고, 대신 PDF 안에 페이지 이미지 형태로 들어갑니다.",
        "완전한 CommonMark/인쇄 CSS 엔진은 아닙니다. 중첩 HTML, 복잡한 다중 리스트, 커스텀 CSS, 이미지 임베드는 이 MVP 범위 밖입니다.",
        "브라우저 메모리를 보호하기 위해 문서 길이와 페이지 수에 상한이 있습니다. 한도를 넘으면 문서를 나누거나 무거운 구간을 줄인 뒤 다시 변환하세요.",
      ],
      faqs: [
        { question: "Markdown이 서버로 업로드되나요?", answer: "아닙니다. 파싱과 PDF 생성은 브라우저에서 실행되며, 변환 자체로는 전송하지 않습니다." },
        { question: "이미지도 PDF에 들어가나요?", answer: "아닙니다. 이미지 Markdown은 링크/대체 텍스트로만 남습니다. 이미지 위주 PDF는 PDF 도구 모음의 이미지 to PDF 탭을 사용하세요." },
        { question: "한글 모양이 기기마다 다른 이유는?", answer: "텍스트를 기기 시스템 폰트 스택으로 그리기 때문입니다. OS·브라우저에 따라 글꼴과 간격이 조금 달라질 수 있습니다." },
        { question: "완전한 Markdown 인쇄 엔진인가요?", answer: "아닙니다. 일상 메모용 범위인 제목, 문단, 목록, 인용, 코드 블록, 구분선, 간단한 표를 다룹니다." },
      ],
    },
  },
  {
