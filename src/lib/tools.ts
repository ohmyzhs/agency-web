...
export const tools: Tool[] = [
  ...
  {
    slug: "pdf-merge-split-reorder",
    title: "PDF Merge, Split & Reorder",
    shortTitle: "PDF Workspace",
    description: "Merge multiple PDFs, select page ranges, and reorder files locally in the browser.",
    category: "file-media",
    tier: 1,
    seoTitle: "PDF Merge, Split & Reorder Tool | Local Browser PDF Editor",
    seoDescription: "Merge PDFs, split by page range, reorder PDF files, and download a new document without uploading files to a server.",
    inputs: [{ label: "PDF files", description: "Upload multiple PDFs and choose page ranges such as all, 1-3, or 1,4,6-8." }],
    outputs: [{ label: "New PDF", description: "Download a merged or extracted PDF created from the selected pages." }],
    examples: [{ label: "Merge reports", input: "Three PDFs, all pages", output: "One combined PDF" }, { label: "Extract pages", input: "Pages 1-2,5", output: "A smaller PDF with selected pages" }],
    explanation: ["This tool uses pdf-lib in the browser to copy pages from uploaded PDFs into a new document.", "Files are processed locally and are not uploaded. Password-protected, damaged, or very large PDFs can still fail because of browser memory limits."],
    faqs: [{ question: "Are PDFs uploaded?", answer: "No. The PDF bytes are read and processed in your browser." }, { question: "Can I drag pages individually?", answer: "This MVP supports file ordering and page ranges. Per-page drag sorting can be added later." }],
    relatedToolSlugs: ["pdf-page-to-image", "image-to-pdf", "image-format-converter"],
    ko: {
      title: "PDF 병합·분할·재정렬",
      shortTitle: "PDF 작업대",
      description: "여러 PDF를 병합하고 페이지 범위를 선택해 새 PDF로 저장합니다. 파일 순서도 브라우저에서 조정합니다.",
      seoTitle: "PDF 병합·분할·재정렬 도구 | 브라우저 로컬 PDF 편집",
      seoDescription: "PDF 병합, 페이지 범위 추출, 파일 순서 재정렬을 서버 업로드 없이 브라우저에서 처리합니다.",
      inputs: [{ label: "PDF 파일", description: "여러 PDF를 업로드하고 all, 1-3, 1,4,6-8 같은 페이지 범위를 지정합니다." }],
      outputs: [{ label: "새 PDF", description: "선택한 페이지로 병합/분할된 PDF를 다운로드합니다." }],
      examples: [{ label: "보고서 병합", input: "PDF 3개, 전체 페이지", output: "하나의 통합 PDF" }, { label: "페이지만 추출", input: "1-2,5 페이지", output: "선택 페이지만 담긴 PDF" }],
      explanation: ["이 도구는 pdf-lib를 사용해 업로드한 PDF의 페이지를 새 문서로 복사합니다.", "파일은 브라우저 안에서 처리됩니다. 암호화/손상 PDF나 매우 큰 PDF는 메모리 한계로 실패할 수 있습니다."],
      faqs: [{ question: "PDF가 서버로 업로드되나요?", answer: "아닙니다. PDF 바이트는 브라우저에서만 읽고 처리합니다." }, { question: "개별 페이지 드래그 정렬도 되나요?", answer: "MVP는 파일 순서와 페이지 범위를 지원합니다. 페이지 단위 드래그 정렬은 이후 확장 가능합니다." }],
    },
  },
  {
    ...
    description: "Render PDF pages into PNG or JPG images locally with pdf.js.",
    category: "file-media", tier: 1,
    seoTitle: "PDF Page to Image Converter | PNG JPG Browser Tool",
    seoDescription: "Convert selected PDF pages to PNG or JPG images locally in your browser with page range and scale options.",
    inputs: [{ label: "PDF and pages", description: "Upload one PDF and select pages such as 1, 1-3, or all." }], outputs: [{ label: "Images", description: "Download individual page images or a ZIP file." }],
    examples: [{ label: "Slide preview", input: "PDF pages 1-3", output: "PNG images for each page" }],
    explanation: ["pdf.js renders each selected page onto Canvas, then exports PNG or JPG.", "This is page rendering, not OCR. Scanned documents remain images."],
    faqs: [{ question: "Does it extract text?", answer: "No. It renders pages as images." }, { question: "Why limit pages?", answer: "Rendering many high-resolution pages can exhaust browser memory, so the MVP caps each run." }],
    relatedToolSlugs: ["pdf-merge-split-reorder", "image-to-pdf", "image-format-converter"],
    ko: { title: "PDF 페이지 이미지 변환", shortTitle: "PDF to 이미지", description: "PDF 페이지를 PNG/JPG 이미지로 브라우저에서 변환합니다.", seoTitle: "PDF 페이지 이미지 변환 | PNG JPG 브라우저 도구", seoDescription: "선택한 PDF 페이지를 PNG 또는 JPG 이미지로 변환하고 ZIP으로 다운로드합니다.", inputs: [{ label: "PDF와 페이지", description: "PDF 하나를 업로드하고 1, 1-3, all 같은 페이지 범위를 지정합니다." }], outputs: [{ label: "이미지", description: "페이지별 이미지 또는 ZIP 파일을 다운로드합니다." }], examples: [{ label: "슬라이드 미리보기", input: "PDF 1-3 페이지", output: "각 페이지 PNG 이미지" }], explanation: ["pdf.js로 선택 페이지를 Canvas에 렌더링한 뒤 PNG/JPG로 내보냅니다.", "OCR 도구가 아니므로 스캔 문서는 이미지 상태로 변환됩니다."], faqs: [{ question: "텍스트도 추출하나요?", answer: "아닙니다. 페이지를 이미지로 렌더링합니다." }, { question: "페이지 수를 제한하는 이유는?", answer: "고해상도 페이지를 많이 렌더링하면 브라우저 메모리를 초과할 수 있어 MVP에서는 제한합니다." }] },
  },
  {
    slug: "image-to-pdf", title: "Images to PDF", shortTitle: "Images to PDF", description: "Combine PNG/JPG images into a PDF with ordering and page-size options.", category: "file-media", tier: 1, seoTitle: "Images to PDF Converter | PNG JPG to PDF", seoDescription: "Create a PDF from multiple PNG or JPG images locally in the browser with ordering, A4, and margin options.", inputs: [{ label: "Images", description: "Upload PNG or JPG images and reorder them." }], outputs: [{ label: "PDF", description: "Download one PDF with one image per page." }], examples: [{ label: "Receipt PDF", input: "Receipt photos", output: "Single PDF for upload" }], explanation: ["Images are embedded into a new PDF using pdf-lib.", "PNG and JPG are supported in the MVP for predictable PDF embedding."], faqs: [{ question: "Does it support WebP?", answer: "Convert WebP to PNG/JPG with the image converter first." }], relatedToolSlugs: ["image-format-converter", "pdf-page-to-image", "pdf-merge-split-reorder"], ko: { title: "이미지 여러 장 PDF 만들기", shortTitle: "이미지 to PDF", description: "PNG/JPG 이미지를 순서대로 묶어 PDF로 만듭니다.", seoTitle: "이미지 PDF 변환 | PNG JPG 여러 장 PDF 만들기", seoDescription: "여러 PNG/JPG 이미지를 브라우저에서 정렬하고 A4/여백 옵션으로 PDF를 생성합니다.", inputs: [{ label: "이미지", description: "PNG 또는 JPG 이미지를 업로드하고 순서를 조정합니다." }], outputs: [{ label: "PDF", description: "이미지 한 장당 한 페이지로 구성된 PDF를 다운로드합니다." }], examples: [{ label: "영수증 PDF", input: "영수증 사진 여러 장", output: "업로드용 단일 PDF" }], explanation: ["pdf-lib로 이미지를 새 PDF 페이지에 삽입합니다.", "MVP는 안정적인 PDF 삽입을 위해 PNG/JPG를 지원합니다."], faqs: [{ question: "WebP도 지원하나요?", answer: "먼저 이미지 포맷 컨버터로 PNG/JPG로 변환한 뒤 사용하세요." }] },
  },
  {
  ...
  {
    slug: "network-diagnostics", title: "Network Diagnostics", shortTitle: "Network", description: "Check public IP and HTTP status/headers with clear browser limitations.", category: "network-diagnostics", tier: 1, seoTitle: "Network Diagnostics Tool | My IP and HTTP Header Check", seoDescription: "Check your public IP and inspect HTTP status and headers while clearly explaining ping, DNS, and browser limitations.", inputs: [{ label: "URL", description: "A public HTTP/HTTPS URL for header checking." }], outputs: [{ label: "Diagnostics", description: "Public IP and server-side HTTP HEAD result." }], examples: [{ label: "Site check", input: "https://oh-my-zhs.com", output: "HTTP status and headers" }], explanation: ["Browsers cannot directly perform ICMP ping, traceroute, or OS-level nslookup.", "This tool implements the feasible subset and labels limitations instead of pretending native network access exists."], faqs: [{ question: "Can it ping?", answer: "Not directly from the browser. ICMP ping requires server or OS-level tooling." }], relatedToolSlugs: ["webhook-request-simulator", "json-yaml-validator", "timestamp-converter"], ko: { title: "네트워크 점검도구", shortTitle: "네트워크", description: "내 공인 IP와 HTTP 상태/헤더를 확인하고 브라우저 한계를 명확히 안내합니다.", seoTitle: "네트워크 점검도구 | 내 IP HTTP 헤더 확인", seoDescription: "내 공인 IP와 HTTP 상태/헤더를 확인하고 ping, DNS, nslookup의 브라우저 제한을 명확히 설명합니다.", inputs: [{ label: "URL", description: "HTTP/HTTPS URL을 입력해 헤더를 확인합니다." }], outputs: [{ label: "진단 결과", description: "공인 IP와 서버 측 HTTP HEAD 결과를 보여줍니다." }], examples: [{ label: "사이트 체크", input: "https://oh-my-zhs.com", output: "HTTP 상태와 헤더" }], explanation: ["브라우저는 ICMP ping, traceroute, OS 수준 nslookup을 직접 수행할 수 없습니다.", "이 도구는 가능한 범위만 구현하고 네이티브 네트워크 접근이 가능한 것처럼 과장하지 않습니다."], faqs: [{ question: "ping도 되나요?", answer: "브라우저에서 직접 ICMP ping은 불가능합니다. 서버 또는 OS 수준 도구가 필요합니다." }] },
  },
  {
