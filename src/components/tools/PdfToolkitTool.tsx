...
type PdfMode = "merge" | "to-image" | "image-to-pdf" | "markdown-to-pdf" | "advanced";
...
export default function PdfToolkitTool() {
  ...
  const copy = useMemo(
    () =>
      locale === "ko"
        ? {
            intro:
              "PDF 관련 작업을 한 페이지에서 처리합니다. 병합/분할/재정렬, PDF 페이지 이미지 변환, 이미지 여러 장 PDF 생성, Markdown → PDF, 텍스트 추출/페이지 삭제/2쪽 모아찍기를 탭으로 전환하세요.",
            local:
            ...
            modes: [
              ...
              {
                id: "markdown-to-pdf" as const,
                label: "Markdown → PDF",
                description: "Markdown 텍스트를 로컬에서 PDF로 렌더링합니다.",
              },
              {
        ...
        : {
            intro:
              "Handle PDF-related workflows from one page: merge/split/reorder PDFs, render PDF pages to images, combine images into a PDF, convert Markdown to PDF, extract text, delete pages, or create 2-up PDFs.",
            local:
            ...
            modes: [
              ...
              },
              {
                ...
                description: "Render Markdown text into a local PDF.",
              },
              {
                id: "advanced" as const,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-5" role="tablist" aria-label={locale === "ko" ? "PDF 도구 선택" : "PDF tool selection"}>
        {copy.modes.map((item) => {
      ...
      <div role="tabpanel" className="rounded-xl border border-border p-4 md:p-5">
        ...
        {mode === "markdown-to-pdf" && <MarkdownToPdfTool />}
