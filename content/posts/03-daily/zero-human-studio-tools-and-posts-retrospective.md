...
이번 개편의 목표는 그 모호함을 줄이는 것이었다. Zero Human Studio를 “실용 도구와 글이 함께 있는 지식형 도구 허브”로 정리하고, 사용자가 바로 쓸 수 있는 기능과 그 기능을 설명하는 콘텐츠를 같은 사이트 안에 배치하려 했다. 이 글은 그 과정에서 어떤 기준으로 사이트 구조를 바꾸고, 어떤 점을 애드센스와 검색 노출 관점에서 고려했는지 남기는 회고다.
...
이 구조는 애드센스만을 위한 형식이 아니다. 하지만 애드센스 관점에서도 중요하다. 사이트가 공사 중처럼 보이지 않고, 메뉴가 비어 있지 않고, 운영자가 어떤 의도로 콘텐츠와 기능을 제공하는지 설명되어야 하기 때문이다.
...
애드센스 승인은 정해진 숫자로 판단되는 일이 아니다. 하지만 경험적으로 얇은 페이지, 미완성 메뉴, 복제된 설명, 정책 페이지 부재, 모바일 사용성 문제는 위험 요소다. 이번 개편에서는 그런 위험을 줄이는 쪽을 우선했다.
...
- 광고가 들어가더라도 입력창, 다운로드 버튼, 결과 영역 바로 옆에는 두지 않는다.

이 기준은 승인만을 위한 체크리스트가 아니라 장기적으로도 사이트 신뢰를 지키는 기준이다.
...
프로덕션 QA에서는 canonical 도메인도 확인해야 했다. 실제 도메인은 하이픈이 들어간 `oh-my-zhs.com`인데, 일부 메타데이터와 sitemap 기본값이 하이픈 없는 도메인을 가리키면 검색 신호가 분산될 수 있다. 이런 작은 불일치는 애드센스와 SEO 모두에서 미리 잡는 편이 좋다.
...
애드센스 관점에서도 이 방향이 더 안전하다. 얇은 페이지를 늘리는 대신 실제 기능과 설명, 정책, 운영 기록을 함께 쌓는 사이트가 되어야 한다. 아직 해야 할 일은 남아 있지만, 이제 Zero Human Studio는 단순한 랜딩 페이지가 아니라 도구와 글이 함께 자라는 사이트의 형태를 갖추기 시작했다.
